...
class serviceServer(Node):
    ...
    def ActivateGripper(self):
        ...
        try:
            ...
            print("ERROR: TCP-IP socket connection timed out during activation!")
            return
        except ConnectionRefusedError:
            print("ERROR: TCP-IP socket connection was refused during activation!")
            return

        try:
            # Send activation command
            SCKT.sendall(b'ACT 1\n')
            reply = SCKT.recv(2**10)
            print("[Activation Reply] ACT 1 ->", reply.decode(errors='ignore'))
            time.sleep(0.5)

        except Exception as e:
            print(f"ERROR during gripper activation: {e}")
    ...
    def ExecuteService(self, request, response):
        ...
        if request.action == "CLOSE":
            ...
            response.message = "CLOSE command successfully sent to Robotiq gripper ("+ignore.decode(errors='ignore')+"). After execution, the gripper is -> " + str(AVERAGE) + "% CLOSED."
            return(response)
            ...
            response.value = GripperPos_STR
            response.average = AVERAGE
            response.message = "OPEN command successfully sent to Robotiq gripper ("+ignore.decode(errors='ignore')+"). After execution, the gripper is -> " + str(AVERAGE) + "% CLOSED."
            return(response)
